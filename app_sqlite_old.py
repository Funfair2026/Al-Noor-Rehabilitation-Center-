...
app.config['SESSION_COOKIE_DOMAIN'] = 'alnoor-funfair-2026.herokuapp.com'
CORS(app)
...
# Require JWT token for API requests
...
def create_default_admin():
    ...
    if count == 0:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?)
        """, (default_user, hashed, "super_admin"))
...
# Require session login for HTML admin pages
...
@app.route('/issue_qr', methods=['POST'])
@require_admin_page_auth
def issue_qr():
    # Require admin to be logged in (session-based)
    if 'username' not in session:
        return redirect('/admin_login')
    ...
    # Validate full name: at least 2 words, minimum 8 characters total 👇
    # import re
    if not re.match(r'^[A-Za-z]{2,}\s+[A-Za-z]{2,}$', full_name) or len(full_name) < 8:
        return render_template('index.html', error="Full name must include real 2 word name with 8 characters minimum.")
    ...
    try:
        ...
        return render_template('index.html', error="Please enter a valid positive amount.")
    try:
    # Use your shared helper to create coupon + QR
        qr_code_img_str, ticket_id = issue_coupon(full_name, amount, admin_username)
    except ValueError as e:
        # Example: visitor already exists → show error on page
        return render_template('index.html', error=str(e))
    # Format issue date
    issue_date = datetime.datetime.now().strftime('%Y-%m-%d')
    # Render QR print page
    return render_template('print_qr.html', visitor_name=full_name, balance=amount, issue_date=issue_date, qr_image=qr_code_img_str, ticket_id=ticket_id)
...
def issue_coupon(full_name, amount, admin_username):
    ...
    if not base_url:
        # Fallback to request.url_root if no env variable
        try:
            base_url = request.url_root.rstrip('/')
        except RuntimeError:
            base_url = "http://localhost:5001"
    qr_code_url = f"{base_url}/authenticator?ticket_id={ticket_id}"   
           ...
           # Check if account is locked
            if user['locked_until']:
                    ...
                    if datetime.datetime.utcnow() < locked_until:
                        log_activity('admin', username, 'login_failed', 'Failed login attempt - account locked')
...
#             # Read credentials from form OR JSON
...
def admin_login():
    try:
        ...
        cursor.execute("SELECT username, password_hash, role FROM admin_users WHERE username = ?", (username,))
        user = cursor.fetchone()
...
@require_admin_page_auth
def admin():
    # Use session instead of JWT token
    username = session.get('username')
    role = session.get('role')

    if not username or role not in ['admin', 'super_admin']:
        #return redirect('/admin_login')
        return jsonify({"redirect": "/admin_login"}), 302
    return render_template('admin.html')
...
def reset_password_page():
     if request.method == 'POST':
        ...
        # In future: verify username exists in DB
...
def change_password():
    if request.method == 'POST':
        ...
        #cursor.execute("SELECT password_hash FROM admin_users WHERE id = 1")
        #current_user = session.get('admin_username')  # from login
        current_user = session.get('username')
        cursor.execute("SELECT password_hash FROM admin_users WHERE username = ?", (current_user,))
        row = cursor.fetchone()
        ...
        import bcrypt
        if not bcrypt.checkpw(old_pw.encode('utf-8'), pw_hash):
            return "❌ Old password is incorrect!"

        new_hash = bcrypt.hashpw(new_pw.encode('utf-8'), bcrypt.gensalt())
        #cursor.execute("UPDATE admin_users SET password_hash = ? WHERE id = 1",
                       #(new_hash,))
...
if __name__ == '__main__':
    ...
    # Local run only — Heroku will ignore this block
    #app.run(debug=False)
